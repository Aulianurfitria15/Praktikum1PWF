            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-black">
                    ...
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <div>
                            <h2 class="text-2xl font-bold text-black tracking-tight">Add Product</h2>
                            <p class="text-sm text-black mt-0.5">Fill in the details to add a new product</p>
                        </div>
                    </div>
                    ...
                    <form action="{{ route('product.store') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name') }}" 
                                ...
                                text-black placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                            @error('name')
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                    ...
                                    <input type="number" id="quantity" name="quantity" value="{{ old('quantity') }}" 
                                        placeholder="0" min="0" 
                                        class="w-full px-4 py-2.5 rounded-lg border text-sm 
                                        {{ $errors->has('quantity') ? 'border-red-500 bg-red-50' : 'border-gray-300 bg-white' }} 
                                        text-black placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                                @error('quantity')
                                    <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="flex items-center justify-end gap-3 pt-2">
                            ...
                            <a href="{{ route('product.index') }}" 
                            class="px-4 py-2.5 rounded-lg border border-gray-400 text-sm font-semibold text-black hover:bg-gray-100 transition">
                                Cancel
                            </a>

                            <button type="submit" 
                                    class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-black text-sm font-bold rounded-lg shadow-sm transition border border-indigo-600">
                                Add Product
                            </button>
                        </div>
                    </form>
